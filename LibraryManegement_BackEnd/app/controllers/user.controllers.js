...
exports.create = async (req, res, next) => {
    ...
    try {
        ...
        return next(
            new ApiError(500, "Lỗi xảy ra khi create user")
        );
    }
}
...
exports.findAll = async (req, res, next) => {
    ...
    try {
        ...
        return next(
            new ApiError(500, "An error orcured while retriveing the users")
        );
    }
...
exports.count = async (req, res, next) => {
    try{
        const userService = new UserService(MongoDB.client);
        let user = await userService.count({ role: "user" }); 
        let manager = await userService.count({ role: "manager" }); 
        let more = await userService.count({ role: "more" });
        return res.send([
            user,
            manager,
            more,
